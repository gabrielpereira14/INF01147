...
%%
programa : decl                                            {$$ = $1; printAST($$,0);  printASTToFile($$, outputFileName);}
    ;
...
dec : type TK_IDENTIFIER ':' lit ';'                        { $$ = astCreate(AST_VARDEC, 0, $1, astCreate(AST_SYMBOL, $2, 0, 0, 0, 0), astCreate(AST_SYMBOL, $4, 0, 0, 0, 0), 0); }
    | type TK_IDENTIFIER '[' LIT_INT ']' vec_init ';'       { $$ = astCreate(AST_VECDEC, 0, $1, astCreate(AST_SYMBOL, $2, 0, 0, 0, 0), astCreate(AST_SYMBOL, $4, 0, 0, 0, 0), $6); }
    | type TK_IDENTIFIER '('paraml')' block                 { $$ = astCreate(AST_FUNDEC, 0, $1, astCreate(AST_SYMBOL, $2, 0, 0, 0, 0), $4, $6); }
    ;
